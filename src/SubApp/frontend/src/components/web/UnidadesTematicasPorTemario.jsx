...
const UnidadesTematicasPorTemario = () => {
    const [unidades, setUnidades] = useState([]);
    const [searchText, setSearchText] = useState('');
    const [filteredUnidades, setFilteredUnidades] = useState([]);

    useEffect(() => {
        const fetchUnidadesTematicasPorTemario = async () => {
            try {
                const response = await axios.get('http://localhost:5000/api/unidad_tematica/temario');
                setUnidades(response.data);
                setFilteredUnidades(response.data);
            } catch (error) {
                console.error('Error al obtener las unidades temáticas por temario:', error);
            }
        };

        fetchUnidadesTematicasPorTemario();
    }, []);
    ...
    return (
        <Container maxWidth="md" style={{ fontFamily: 'Georgia' }}>
            <Typography variant="h5" component="h2" gutterBottom style={{ display: 'flex', alignItems: 'center', fontFamily: 'Georgia' }}>
                <SchoolIcon style={{ marginRight: '10px' }} /> Unidades Temáticas por Temario
            </Typography>
            <Box mb={2}>
                <TextField
                    fullWidth
...
export default UnidadesTematicasPorTemario;